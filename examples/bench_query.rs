...
fn main() {
    ...
    println!(
        "== bench_query: N={n_codes} codes × M={m_days} days = {} rows ==",
        n_codes * m_days
    );
    ...
    for c in 0..n_codes {
        ...
        store
            .write("RawDailyBar", &code, &recs, Some(m_days))
            .unwrap();
    }
    ...
    run_bin(
        "D4 close>45 && ma(volume,5)>1e6",
        "close>45 && ma(volume,5)>1e6",
    );
